...
class CollapsibleDistribution(object):
    ...
    def validate_target_model(target_model):
        """
        Input validation. Checks for standard structure items. Model-dependent
        checks should be implemented in their respective classes. See
        NormalInverseWishart for an example.
        """
        ...
        if d is None:
            raise KeyError('Target model should have key "d" (int, '
                           'num_dimensions')
        if not isinstance(d, int):
            raise TypeError('Target["d"] should be an int.')

        if parameters is None:
            raise KeyError('Target model should have key "parameters" (list '
        ...
        if assignment is None:
            raise KeyError('Target model should have key "assignment" (data '
                           'partition)')
...
class NormalInverseWishart(CollapsibleDistribution):
    """
    Mulitvariate Normal likelihood with multivariate Normal prior on mean and
    Inverse-Wishart prior on the covaraince matrix.
    ...
    def with_vague_prior(cls, target_model):
        """
        Initialize using a set of vague hyperparamers derived from the target
        model.
    ...
    def log_marginal_likelihood(self, X):
        ...
        return fniw.niw_ml(X, self.lambda_0, self.mu_0, self.kappa_0,
                           self.nu_0, self.log_z)
    ...
    def log_posterior_predictive(self, Y, X):
        ...
        # params_n = self.update_parameters(X, self.mu_0, self.lambda_0,
        #                                   self.kappa_0, self.nu_0, self.d)
        # params_m = self.update_parameters(YX, self.mu_0, self.lambda_0,
        #                                   self.kappa_0, self.nu_0, self.d)
        # log_z_n = self.calc_log_z(*params_n)
        # log_z_m = self.calc_log_z(*params_m)

        # return log_z_m - log_z_n - LOG2PI*(self.d/2)
        return fniw.niw_pp(Y, X, self.lambda_0, self.mu_0, self.kappa_0,
                           self.nu_0)

    def draw_data_from_prior(self, n):
        return np.random.multivariate_normal(
            self.mu_0, self.lambda_0/self.kappa_0, n)
    ...
    def validate_target_model(target_model):
        ...
        for params_k in target_model['parameters']:
            if len(params_k) != 2:
                raise ValueError("Each parameter tuple must have two entries "
                                 "(mean, variance/covaraince)")
            m, s = params_k
            ...
            if not isinstance(s, np.ndarray):
                raise TypeError('Each variance/covariance must be a numpy '
                                'array')
            if not (s.shape[0] == target_model['d'] and
                    s.shape[1] == target_model['d']):
                raise TypeError('Each coavariance matrix should be d rows by '
                                'd coolumns')
...
def do_log_crp(H, n, crp_alpha):
    """
    Log probability of a partition of n objects given the concentraion
    parameter, alpha, under CRP.

    Parameters
    ----------
    H : numpy.ndarry<float>
        H[i] is the number of items assigned to category i
    n : int
        The number of datapoints in H
    crp_alpha : float
        CRP alpha
    ...
    """
    K = len(H)
    logp = np.sum(gammaln(H)) + K*log(crp_alpha) + gammaln(crp_alpha) -\
        gammaln(n + crp_alpha)
...
def _do_log_marginal(model, X, Z):
    """
    Calcualtes the marinal likelihood of the data X, partitioned by Z given
    model.
    """
